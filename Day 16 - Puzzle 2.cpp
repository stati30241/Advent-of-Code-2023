...
/*
* If the brute force solution doesn't work, you
* simply aren't using enough brute force, or 
...
int main() {
	...
	// Calculates the different starting positions
	std::vector<Light> startingPositions;
	for (int j = 0; j < map.size(); ++j) {
		startingPositions.push_back({ { 0, j }, { 1, 0 } });
		startingPositions.push_back({ { static_cast<int>(map.at(j).size()) - 1, j }, { -1, 0 } });
	}
	for (int i = 0; i < map.at(0).size(); ++i) {
		startingPositions.push_back({ { i, 0 }, { 0, 1 } });
		startingPositions.push_back({ { i, static_cast<int>(map.size()) - 1 }, { 0, -1 } });
	}
	int maxEnergizedTiles = 0;

	// Checks every single starting position
	for (const auto& startPos : startingPositions) {
		// Simulates the beam of light going into the contraption
		std::vector<Light> beamsOfLight{ startPos };
		...
		// Compares it to the max
		if (totalEnergizedTiles > maxEnergizedTiles) maxEnergizedTiles = totalEnergizedTiles;
	}

	std::cout << maxEnergizedTiles << std::endl;
